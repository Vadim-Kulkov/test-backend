...
2. Из модели `BudgetType` через миграцию БД убрать значение `Комиссия`, заменив его на `Расход`. В миграциях нет такой модели, соответственно, заменять нечего
3. Добавить таблицу `Author` - автор внесения записи. - Готово, немного добавил логики в тесты под эти пункты
